<script>
  ...
  export function abort(reason) {
    if (checkInitialized()) _managerInstance.abort(reason);
  }
  ...
  export async function callMain(args) {
    if (checkInitialized()) await _managerInstance.callMain(args);
  }

  export async function pauseMainLoop() {
    if (checkInitialized()) await _managerInstance.pauseMainLoop();
  }

  export async function resumeMainLoop() {
    if (checkInitialized()) await _managerInstance.resumeMainLoop();
  }

  export async function requestFullscreen() {
    if (checkInitialized()) return await _managerInstance.requestFullscreen();
  }

  export async function exitFullscreen() {
    if (checkInitialized()) return await _managerInstance.exitFullscreen();
  }
...
>
  ...
  {:else}
    ...
    <textarea bind:this={_consoleElement} class:hide={!console} rows="8"
    ></textarea>
